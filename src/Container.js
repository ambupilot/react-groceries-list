	...
	render() {
		...
		const addGrocery = () => {
			if (this.state.groceryInput !== "") {
				this.setState({
					groceryInput: "",
					groceryItems: [...this.state.groceryItems, this.state.groceryInput]
				});
			}
		};
		const handleChange = e => {
			...
			change.id = this.state.groceryItems.length + 1;
			this.setState({
				groceryInput: change,
		...
		return (
			<div className="container">
				<div className="groceries-list groceries">
					...
					<InputField handleChange={handleChange} addGrocery={addGrocery} />
					<GroceryList
						groceryItems={this.state.groceryItems}
